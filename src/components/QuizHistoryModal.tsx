...
const QuizHistoryModal: React.FC<QuizHistoryModalProps> = ({ 
  ...
  const scorePercentage = quiz.results?.punteggio_totale && quiz.questions?.length
    ? Math.round((quiz.results.punteggio_totale / quiz.questions.length) * 100)
    : quiz.results?.total_points && quiz.results?.max_points
    ? Math.round((quiz.results.total_points / quiz.results.max_points) * 100)
    : null;
    
  // Get points data if available
  const totalPoints = quiz.results?.total_points || 0;
  const maxPoints = quiz.results?.max_points || quiz.questions?.length || 0;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{quiz.title || t('quizResults')}</DialogTitle>
          <DialogDescription>
            {quiz.results && (
              <div className="mt-2 space-y-1">
                <div className="text-lg font-bold">
                  {scorePercentage !== null ? `${scorePercentage}% ${t('score')}` : t('noScoreAvailable')}
